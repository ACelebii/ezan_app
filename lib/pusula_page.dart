...
class PusulaPage extends StatefulWidget {
  const PusulaPage({super.key});

  @override
  State<PusulaPage> createState() => _PusulaPageState();
}

class _PusulaPageState extends State<PusulaPage> {
  double _cumulativeHeading = 0;
  double _lastRawHeading = 0;
  double _qiblaAngle = 147;
  ...
  Future<void> _updateLocationAndQibla() async {
    try {
      // 1. Önce Konum Servisi Açık mı Kontrol Et
      ...
      // 3. Konumu Al (Sanal cihazlar yavaş olabileceği için timeLimit'i kaldırdık)
      ...
      if (mounted) {
        setState(() {
          _currentPosition = pos; // Harita için konumu kaydet
        ...
        onTap: () {
          if (_currentPosition != null) {
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    QiblaMapPage(userPosition: _currentPosition!),
              ),
            );
          } else {
  ...
  Widget build(BuildContext context) {
    ...
    double displayHeading = (_cumulativeHeading % 360 + 360) % 360;
    double diff = (_qiblaAngle - displayHeading + 360) % 360;
              ...
              child: Center(
                child: Stack(
                  ...
                  children: [
                    ...
                    _build3DBezel(),
                    _buildCompassBody(displayHeading, isAligned),
  ...
  Widget _buildCompassBody(double heading, bool isAligned) {
    return AnimatedRotation(
      turns: -_cumulativeHeading / 360,
      duration: const Duration(milliseconds: 600),
